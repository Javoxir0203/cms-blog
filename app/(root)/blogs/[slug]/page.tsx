...
import SHareBtnsCard from '../../-components/share-btns';

export async function generateMetadata({ params }: { params: { slug: string } }) {
	const blog = await getDetailedBlog(params.slug);

	return { title: blog.title, description: blog.description, openGraph: { images: blog.image.url } };
}
		...
		<div className='pt-[15vh] max-w-5xl mx-auto'>
			...
			<div className='flex mt-6 gap-6 items-center max-md:flex-col'>
				...
				<div className='flex-1 flex flex-col space-y-4'>
					...
					<Link href={`/author/${blog.author.id}`} className='flex items-center gap-2 hover:text-blue-500 underline transition-colors'>
